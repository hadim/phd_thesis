...
Quand on compare les états de cohérence sur plusieurs cellules dans différentes conditions (@fig:coherence_stats) on observe que le mutant kinésine-8 passe plus de temps dans un état incohérent que des cellules sauvages. Plus précisément le mutant kinésine-8 semble passer plus de temps dans l'état incohérent AP-AP alors que l'état P-P semble être le même que dans les cellules sauvages.

![Temps passé dans les différents états de cohérence dans différentes conditions. WT désigne une cellule sauvage. _klp6Δ_, _klp5Δ_ et _klp56Δ_ désigne différents mutants de la kinésine-8 qui possède tous le même phénotype (voir @sec:article pour plus de détails). TBZ désigne une drogue, le thiabendazole qui inhibe les oscillations des chromosomes en métaphase. Le nombre de cellule utilisés pour cette analyse est n = 24 pour WT, n = 18 pour _klp6Δ_, n = 12 pour _klp5Δ_, n = 26 pour _klp56Δ_, n = 19 pour WT+TBZ, n = 14 pour _klp6Δ_+TBZ.](figures/results/imaging/coherence_stats.png "Temps passé dans les différents états de cohérence dans différentes conditions"){#fig:coherence_stats}
...
La distance inter-kinétochore entre les deux kinétochores peut être un bon moyen que quantifier la tension exercé sur les kinétochores par les microtubules. De cette façon en observant la distance inter-kinétochore en fonction de l'état de cohérence dans différentes conditions, on voit que de manière assez subtile la distribution des distances pour un état cohérent (AP-P) a une moyenne de à peu près de 400nm dans toutes les conditions. Elle semble par ailleurs très légèrement supérieure pour l'état incohérent P-P et très légèrement inférieure pour l'état incohérent AP-AP. Bien que ces résultats ne soit pas significatif, ils pourraient supporter l'idée qu'un mouvement AP implique une force de poussée tandis qu'un mouvement P implique une force de traction sur le kinétochore.

![Distance inter-kinétochore en fonction de l'état de cohérence dans différentes conditions. La barre rouge des violin plots désigne la moyenne. Les commentaires de la figure précédente s'appliquent à cette figure.](figures/results/imaging/coherence_stretch.png "Distance inter-kinétochore en fonction de l'état de cohérence dans différentes conditions"){#fig:coherence_stretch}

Cette dernière observation nécessite d'être confirmer car la mesure de la distance inter-kinétochore dans ce système pourrait comporter un biais important. En effet le mutant Cen2-GFP possède une sonde fluorescente au niveau de la partie péri-centromérique de la chromatide, donc proche mais pas tout à fait au même endroit que le kinétochore. La distance entre les deux positions a été estimée à 125nm [@Gay2012a] ce qui est bien plus que la différence observée. De plus la sonde ne se situe pas dans l'axe kinétochore-kinétochore que qui signifie qu'une traction ou une poussée sur cet axe pourrait bien être invisible en mesurant la distance entre les sonde Cen2-GFP.
...
L'outil communément utilisé pour l'étude de ces processus est la mesure du « Mean Square Displacement » (MSD) qui représente l'étendue spatiale explorée (caractérisé par une aire dans le cas d'un mouvement en deux dimensions) en fonction de différente intervalle de temps $\tau$.

Si on suppose ces processus stationnaires, il est possible de modéliser ces processus tel que :
...
Où $R_c$ est la rayon dans lequel la particule est confiné, $d$ est le nombre de dimensions et $D$ le coefficient de diffusion.
...
Cette approche est appelé modélisation basé sur les données (« data-driven modeling » en anglais). Toutes la difficulté est de trouver la bonne technique pour remonter aux équations à partir des données. Récemment un article publiée par Monier et al. [@Monnier2012] propose une approche basé sur les statistiques bayésiennes afin de prédire pour différents types de mouvement le processus à l'origine ainsi que les paramètres associés.
...
On peut aussi observer que les trajectoires avec TBZ sont très largement sous diffusives comparées aux trajectoires sans TBZ. Ceci qui confirme qu'une force net plus faible est appliqué dans ces conditions dû à un attachement plus faible des kinétochores aux microtubules.
...
La visualisation en log-log (@fig:msds-log) permet de voir de manière plus précise ce qui se passe aux temps court. Il est ainsi possible d'observer que toutes les MSD pourraient posséder trois temps caractéristiques :

- Un premier allant juqu'à quelques centaines de microseconde pourrait traduire l'effet des différents processus aléatoires sur le mouvement (mouvement brownien, bruit blanc de la mesure, etc).

- De 200ms à 2-3s, la pente chute brusquement, ce qui pourrait être la signature de la mesure de Cen2-GFP qui bougerait de la même manière qu'une particule confinée dû à son éloignement au point où s'exerce les forces au niveau du kinétochore.

- De 2s à 50s, la pente augmente ce qui pourrait traduire l'effet du la force appliqué au niveau des kinétochores. Cette pente serait alors caractéristique d'un mouvement dirigé.

Il est d'ailleurs très intéressant de remarquer que dans les MSD avec TBZ, la troisième pente (de 2s à 50s) est quasiment égale à la seconde pente (de 200ms à 2-3s). Cela pourrait signifier que les kinétochores très faiblement en présence de TBZ sont très peu soumis à un processus de mouvement dirigé comme le confirme l'analyse des oscillations en @sec:article.
...
Des analyses plus précises sont nécessaires pour pouvoir conclure sur les différents types de phénomène en jeu dans le mouvement des chromosomes. Il serait bien notamment comparer le mouvement de Ndc80-GFP (situé au niveau du kinétochore) avec celui de Cen2-GFP.

Il pourrait être par la suite utile de modéliser à partir de ces différentes mesures les modèles de mouvement vu dans la section précédente. L'approche bayésienne semble être une approche intéressante dans ce type de problème [@Monnier2012].
