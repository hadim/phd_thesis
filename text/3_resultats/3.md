...
L'exécution d'un grand nombre de simulations peut prendre du temps, c'est pour cela que `kt_simul` possède aussi des fonctions permettant la répartition de différentes simulations sur différents coeurs de la machine qui les exécute. La parallélisation permet une réduction drastique des temps de calcul sans avoir à requérir à un puissant cluster, souvent cher et difficile d'accès. Ainsi une station de travail classique, équipé de 16 coeurs Intel Xeon E5-2650 cadencé à 2GHz, est capable d'exécuter plus de 2000 simulations standards tandis que la version non parallélisée exécute seulement 200 simulations.

![Différence de vitesse d'exécution théorique entre une implémentation parallélisée et une implémentation linéaire. Un algorithme parallélisé (en bleu) utilise les différents coeurs de la machine pour exécuter plusieurs simulations en même temps. Donc une machine possédant deux coeurs exécute en théorie les quatres simulations deux fois plus vite qu'un algorithme non parallélisé (en rose).](figures/results/parallel.png){#fig:parallel}
...
On rappelle que le modèle initialement développé [@Gay2012a] ne contient pas de structure pouvant s'apparenter aux microtubules _in vivo_.  En effet, chaque site d'attachement peut se trouver dans deux états; soit « ON » est produire une force en direction du pôle, soit « OFF » est ne produire aucune force (@fig:two_states). Le passage entre les deux états se fait de manière stochastique avec en plus deux mécanismes de corrections qui peuvent modifier les constantes d'attachement et de détachement (voir @Gay2012a pour le fonctionnement de ces deux mécanismes de corrections).

![Modèle d'attachement à deux états. Le passage d'un état à l'autre se fait de manière stochastique et est régulé par deux mécanismes de corrections $\beta$ et $d_\alpha$.](figures/results/two_states.png){#fig:two_states}

L'idée du modèle d'attachement à trois états est d'ajouter un niveau de flexibilité dans la dynamique des attachements et donc aussi dans la dynamique des forces appliquées aux chromosomes.


![Modèle d'attachement à trois états. Le passage d'un état à l'autre se fait de manière stochastique et est régulé par deux mécanismes de corrections $\beta$ et $d_\alpha$.](figures/results/three_states.png){#fig:three_states}
