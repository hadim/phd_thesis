...
Scp25-Spc24, possède un domaine s'attachant à la plaque externe du
kinétochore.
...
al., \protect\hyperlink{ref-Miranda2005}{2005}; Westermann et al.,
\protect\hyperlink{ref-Westermann2006}{2006}).
...
\caption{\label{fig:jaqaman}Les deux matrices de coût du LAP tracker.
\textbf{A}. Matrice de coût contentant l'ensemble des liens possibles
entre deux blobs pour deux pas de temps successifs \(t\) et \(t+1\). Il
existe donc autant de cette première matrice que de pas de temps dans la
trajectoire. Le bloc en bas à droite (LRB) est un bloc auxiliaire requis
pour satisfaire des contraintes topologiques de l'algorithme.
\textbf{B}. Matrice de coût contrôlant la fermeture des trous, la fusion
et la séparation des trajectoires. Adapté de Jaqaman et al.
(\protect\hyperlink{ref-Jaqaman2008}{2008}).}
\end{figure}
...
\caption{\label{fig:workflow}Workflow de reconstruction des
trajectoires. Deux étapes nécessitent une intervention manuelle. Suite à
la détection de blobs (peaks) et après la reconstruction automatique des
trajectoires.}
\end{figure}
...
\caption{\label{fig:coherence-stats}Temps passé dans les différents
états de cohérence dans diverses conditions. WT désigne une cellule
sauvage. \emph{klp6Δ}, \emph{klp5Δ} et \emph{klp56Δ} désigne différents
mutants de la kinésine-8 qui possèdent tous le même phénotype (voir
...
métaphase. Le nombre de cellules utilisées pour cette analyse est n = 24
...
Ainsi il est introduit dans le système un mécanisme de communication
entre les deux kinétochores frères et on suppose alors que les
...
une rang de valeurs acceptable compris entre \(10^{-3} s^{-1}\) et
\(1 s^{-1}\).

Cependant, le nombre de paramètres libres ajouté par ce modèle a rendu
difficile son optimisation afin d'obtenir des simulations stables. Pour
le moment aucun jeux de paramètre n'a encore permis de reproduire des
...
suffisant pour reproduire des trajectoires observées \emph{in vivo} et
...
testées \emph{in silico} afin d'évaluer leur pertinences.
...
permettant la congression, est l'homologue de la kinésine-8, appelé
Kif18a (Stumpff et al., \protect\hyperlink{ref-Stumpff2008}{2008}). Le
modèle explique que la grande processivité de la kinésine-8 (Gupta et
al., \protect\hyperlink{ref-Gupta2006}{2006}; Varga et al.,
...
par le fait que Klp5 et Klp6 agissent en formant un hétérodimère
...
Les premiers résultats montrent que cette hypothèse alternative arrive à
...
avec la kinésine-7 absente chez ce dernier (Grissom et al.,
...
des chromosomes sans bien reproduire les oscillations observées \emph{in
vivo}.
...
fidèle le mouvement des chromosomes dans un cadre général de la
ségrégation des chromosomes (Gay et al.,
\protect\hyperlink{ref-Gay2012a}{2012}).
...
oscillations des chromosomes ont presque toujours été faites sur des
